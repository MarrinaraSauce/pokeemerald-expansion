...
static void DexNavDrawIcons(void)
{
    ...
    DrawDexNavSearchMonIcon(species, &sDexNavSearchDataPtr->iconSpriteId, GetSetPokedexFlag(SpeciesToNationalPokedexNum(species, TRUE), FLAG_GET_CAUGHT));
...
static void DexNavUpdateSearchWindow(u8 proximity, u8 searchLevel)
{
    ...
    if (sDexNavSearchDataPtr->hiddenSearch && !GetSetPokedexFlag(SpeciesToNationalPokedexNum(sDexNavSearchDataPtr->species, TRUE), FLAG_GET_SEEN))
...
static u8 DexNavGetAbilityNum(u16 species, u8 searchLevel)
{
    ...
    if (genAbility
            ...
            && GetSetPokedexFlag(SpeciesToNationalPokedexNum(species, TRUE), FLAG_GET_CAUGHT))
    {
...
{
    u32 i;
    enum NationalDexOrder dexNum = SpeciesToNationalPokedexNum(species, TRUE);
    ...
    case 0: //land
        for (i = 0; i < LAND_WILD_COUNT; i++)
        {
            if (SpeciesToNationalPokedexNum(sDexNavUiDataPtr->landSpecies[i], TRUE) == dexNum)
                return TRUE;
        }
        break;
    case 1: //water
        for (i = 0; i < WATER_WILD_COUNT; i++)
        {
            if (SpeciesToNationalPokedexNum(sDexNavUiDataPtr->waterSpecies[i], TRUE) == dexNum)
                return TRUE;
        }
        break;
    case 2: //hidden
        for (i = 0; i < HIDDEN_WILD_COUNT; i++)
        {
            if (SpeciesToNationalPokedexNum(sDexNavUiDataPtr->hiddenSpecies[i], TRUE) == dexNum)
                return TRUE;
        }
        break;
    default:
...
static void TryDrawIconInSlot(u16 species, s16 x, s16 y)
{
    ...
    else if (!GetSetPokedexFlag(SpeciesToNationalPokedexNum(species, TRUE), FLAG_GET_SEEN))
